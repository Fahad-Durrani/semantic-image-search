...
Images are embedded once with **Apple's official [MobileCLIP-S2](https://github.com/apple/ml-mobileclip)**.
A Flask server then encodes your text query (or an uploaded image) with the same
model and ranks the library by cosine similarity.
...
```
                 build_index.py (offline, once)
   images_repo/  ─────────────────────────────►  cache/embeddings.npz
   (7,800 imgs)      MobileCLIP-S2 encode_image     (N×512, L2-normalised)
                     + L2-normalise

                 app.py (serving)
   "a red car"  ──► encode_text ──► 512-d vec ──►  dot product vs. every row
                    (same model)                   ──► top-k ──► JSON + web UI
```
...
| **The MobileCLIP-S2 checkpoint** | ~380 MB, downloaded once (see below). Not in this repo. |
...
Download **`mobileclip_s2.pt`** into a `checkpoints/` folder:

```powershell
mkdir checkpoints
# From: https://docs-assets.developer.apple.com/ml-research/datasets/mobileclip/mobileclip_s2.pt
Invoke-WebRequest `
...
### 6. Build the embedding index (one time, ~10 min on CPU)

```powershell
python build_index.py
```

This writes `cache/embeddings.npz` and shows a `tqdm` progress bar. Corrupt files
are skipped automatically. Re-run only when the contents of `images_repo/` change
(or use the in-app **Rebuild Index** button for an incremental update).
...
| `GET /search` | Text search | `q` (query, supports `-negative` terms), `k` (1–50, default 10), `color` |
| `GET /similar` | Nearest neighbours of an indexed image | `img` (filename), `k` |
| `POST /search_by_image` | Search by an uploaded image | multipart `image` file, `k` |
| `GET /suggest` | Autocomplete | `q` (prefix ≥2 chars), `limit` |
| `GET /colors` | List available colour-filter names | — |
| `POST /rebuild` | Start an incremental re-index | — |
| `GET /rebuild_stream` | Server-sent-events progress for a rebuild | — |
| `GET /images/<filename>` | Serve an image from `images_repo/` | — |

**Example — text search with a negative term:**

```powershell
Invoke-RestMethod -Uri "http://127.0.0.1:5000/search?q=beach -people&k=5"
```
...
```json
{
  "query": "lion", "positive": "lion", "negatives": [], "k": 3,
  "results": [
...
```
semantic-image-search/
├── app.py              # Flask server + embedded single-page frontend
├── build_index.py      # Offline indexer → cache/embeddings.npz
...
├── cache/              # embeddings.npz, colors.json, suggestions.json, skip_list.json
└── ml-mobileclip/      # Apple's package (cloned, installed editable)
```

Paths are hard-coded in `app.py` and `build_index.py` — keep them in sync if you
move things. See **[CLAUDE.md](CLAUDE.md)** for architecture details and the
"things that will bite you" list (e.g. `model.eval()` is mandatory — MobileCLIP
has batchnorm).
...
- **`FileNotFoundError: checkpoints/mobileclip_s2.pt`** — you skipped step 4.
- **`No module named 'mobileclip'`** — run step 2 (`pip install -e ./ml-mobileclip --no-deps`).
- **Cache not found / empty results** — run `python build_index.py` (step 6).
...
- Model: **[Apple MobileCLIP](https://github.com/apple/ml-mobileclip)** (MobileCLIP-S2). Checkpoint © Apple, under Apple's licence.
